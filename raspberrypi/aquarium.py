...
class Aquarium:
    ...
    def set_temp_heater_off(self, temp):
        ...
        if temp > 40:
            log.error(
                "Set Heater-off temperature %d failed, because the setting value is too much high above 40 degrees", temp)
            return
    ...
    def reload_cfg(self):
        ...
        obj=cfg['pumpext']
        if obj is not None:
            self.pumpext_relay=Relay(
                obj['relay']['pin'], obj['relay']['reverse'])
        # set uvlight
        obj=cfg['uvlight']
        if obj is not None:
            self.uvlight_relay=Relay(
                obj['relay']['pin'], obj['relay']['reverse'])
        # set temp_sensor
        obj=cfg['temp_sensor']
        if obj is not None:
